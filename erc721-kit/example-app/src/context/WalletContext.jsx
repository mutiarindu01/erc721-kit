...
export function WalletProvider({ children }) {
  ...
  // Helper functions
  const setLoading = (loading) => dispatch({ type: WALLET_ACTIONS.SET_LOADING, payload: loading });
  const setError = (error) => dispatch({ type: WALLET_ACTIONS.SET_ERROR, payload: error });
  const clearError = () => dispatch({ type: WALLET_ACTIONS.SET_ERROR, payload: null });
  ...
  // Connect wallet
  const connectWallet = useCallback(async (walletType = WALLET_TYPES.METAMASK) => {
    try {
      setLoading(true);
      clearError();
      ...
      dispatch({ type: WALLET_ACTIONS.SET_CHAIN_ID, payload: network.chainId });
      dispatch({ type: WALLET_ACTIONS.SET_BALANCE, payload: ethers.utils.formatEther(balance) });
  ...
  // Add token to wallet
  const addTokenToWallet = useCallback(async (tokenAddress, tokenSymbol, tokenDecimals = 18, tokenImage) => {
    try {
      if (!state.provider) {
        throw new Error('No provider available');
      }
