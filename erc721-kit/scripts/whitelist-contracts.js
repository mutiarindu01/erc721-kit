...
async function whitelistContract(contractInstance, contractAddress, contractName) {
  try {
    console.log(`Whitelisting ${contractName} (${contractAddress})...`);
    
    // Check if already whitelisted
    const isWhitelisted = await contractInstance.whitelistedContracts(contractAddress);
    ...
    const tx = await contractInstance.setContractWhitelist(contractAddress, true);
    console.log(`Transaction sent: ${tx.hash}`);
    
    const receipt = await tx.wait();
    console.log(`✓ ${contractName} whitelisted successfully (Gas used: ${receipt.gasUsed})`);
...
async function main() {
  ...
  try {
    ...
    const marketplaceAddress = deploymentInfo.contracts.ERC721Marketplace.address;
    const escrowAddress = deploymentInfo.contracts.ERC721Escrow.address;
    
    console.log(`\nMarketplace: ${marketplaceAddress}`);
    console.log(`Escrow: ${escrowAddress}`);
    
    const marketplace = await ethers.getContractAt("ERC721Marketplace", marketplaceAddress, signer);
    const escrow = await ethers.getContractAt("ERC721Escrow", escrowAddress, signer);
    
    // If no specific contracts provided, try to whitelist deployed MockNFT
    let contractsToWhitelist = NFT_CONTRACTS;
    
    if (contractsToWhitelist.length === 0 || contractsToWhitelist[0].address === "0x...") {
      console.log("\n⚠️ No specific contracts configured. Looking for deployed MockNFT...");
      
      // Check if MockNFT was deployed
      const mockNFTFile = path.join(__dirname, "../deployments", `${networkName}-mock-nft.json`);
      ...
      console.log(`const NFT_CONTRACTS = [
  {
    ...
    for (const contract of contractsToWhitelist) {
      if (!ethers.utils.isAddress(contract.address)) {
        console.log(`❌ Invalid address for ${contract.name}: ${contract.address}`);
    ...
    // Check for failures
    const marketplaceFailures = Object.values(results.marketplace).filter(v => !v).length;
    const escrowFailures = Object.values(results.escrow).filter(v => !v).length;
    ...
    // Save results
    const resultsFile = path.join(__dirname, "../deployments", `${networkName}-whitelist-results.json`);
    const resultsData = {
...
async function checkWhitelistStatus(contractAddress, deploymentInfo) {
  ...
  try {
    ...
    const marketplaceWhitelisted = await marketplace.whitelistedContracts(contractAddress);
    const escrowWhitelisted = await escrow.whitelistedContracts(contractAddress);
    
    console.log(`Marketplace: ${marketplaceWhitelisted ? '✅ Whitelisted' : '❌ Not whitelisted'}`);
    console.log(`Escrow: ${escrowWhitelisted ? '✅ Whitelisted' : '❌ Not whitelisted'}`);
...
if (process.argv.length > 2) {
  ...
  if (command === "check" && process.argv.length >= 4) {
    ...
    loadDeploymentInfo(network.name)
      .then(deploymentInfo => checkWhitelistStatus(contractAddress, deploymentInfo))
      .then(() => process.exit(0))
      .catch(error => {
        console.error(error);
        process.exit(1);
