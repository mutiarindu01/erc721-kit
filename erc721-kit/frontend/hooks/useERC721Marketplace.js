...
const useERC721Marketplace = (marketplaceAddress, escrowAddress, options = {}) => {
  const {
    autoConnect = true,
  ...
  const connectWallet = useCallback(async () => {
    try {
      ...
      if (marketplaceAddress) {
        const marketplace = new ethers.Contract(marketplaceAddress, ERC721_MARKETPLACE_ABI, signer);
        setMarketplaceContract(marketplace);
      }

      if (escrowAddress) {
        const escrow = new ethers.Contract(escrowAddress, ERC721_ESCROW_ABI, signer);
  ...
  const listItem = useCallback(async (nftContract, tokenId, price, durationDays = 7) => {
    ...
    try {
      ...
      const tx = await marketplaceContract.listItem(nftContract, tokenId, price, duration);
      await tx.wait();
  ...
  const buyItem = useCallback(async (listingId, price) => {
    if (!marketplaceContract) throw new Error('Marketplace contract not initialized');
    ...
    try {
      ...
      // Refresh data
      await Promise.all([fetchListings(), fetchUserListings(), fetchMarketplaceStats()]);
      return tx;
    } catch (error) {
      setError(error.message);
      throw error;
  ...
  const updateListing = useCallback(async (listingId, newPrice) => {
    if (!marketplaceContract) throw new Error('Marketplace contract not initialized');
  ...
  const createOffer = useCallback(async (nftContract, tokenId, amount, durationDays = 7) => {
    if (!marketplaceContract) throw new Error('Marketplace contract not initialized');
    ...
    try {
      ...
      const tx = await marketplaceContract.createOffer(nftContract, tokenId, duration, { value: amount });
      await tx.wait();
  ...
  // Escrow functions
  const createEscrow = useCallback(async (buyer, nftContract, tokenId, price, deadlineDays = 7) => {
    ...
    try {
      ...
      const tx = await escrowContract.createEscrow(buyer, nftContract, tokenId, deadline, { value: price });
      await tx.wait();
  ...
  const approveEscrow = useCallback(async (escrowId) => {
    if (!escrowContract) throw new Error('Escrow contract not initialized');
  ...
  const initiateDispute = useCallback(async (escrowId) => {
    if (!escrowContract) throw new Error('Escrow contract not initialized');
  ...
  const approveNFT = useCallback(async (nftContract, tokenId, spender) => {
  ...
  const setApprovalForAll = useCallback(async (nftContract, operator, approved = true) => {
    if (!signer) throw new Error('Wallet not connected');
    ...
    try {
      const nftContractInstance = new ethers.Contract(nftContract, ERC721_ABI, signer);
      const tx = await nftContractInstance.setApprovalForAll(operator, approved);
      await tx.wait();
      return tx;
    } catch (error) {
      setError(error.message);
  ...
  const getNFTMetadata = useCallback(async (nftContract, tokenId) => {
  ...
  }, [marketplaceContract, userAddress, fetchUserListings, fetchUserOffers, fetchMarketplaceStats]);
  ...
  }, [enableRealTimeUpdates, pollInterval, isConnected, isLoading, fetchUserListings, fetchUserOffers, fetchEscrows, fetchMarketplaceStats]);
  ...
  return {
    ...
    refresh: useCallback(() => {
      ...
      fetchEscrows();
      fetchMarketplaceStats();
    }, [fetchUserListings, fetchUserOffers, fetchEscrows, fetchMarketplaceStats])
  };
};
...
export const useNFTApproval = (nftContract, tokenId, spender) => {
  ...
  const checkApproval = useCallback(async () => {
    if (!nftContract || !tokenId || !spender || typeof window.ethereum === 'undefined') return;
    ...
    try {
      ...
      const approvedForAll = await contract.isApprovedForAll(await contract.ownerOf(tokenId), spender);
...
export const useNFTOwnership = (nftContract, tokenId, userAddress) => {
  ...
  const checkOwnership = useCallback(async () => {
    if (!nftContract || !tokenId || typeof window.ethereum === 'undefined') return;
