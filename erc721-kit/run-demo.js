      ...
      try {
        ...
        if (!proc.killed) {
          ...
          try {
            ...
            log('⚠️  Please open http://localhost:' + port + ' in your browser', 'yellow');
          }
...
async function showInfo() {
  ...
  try {
    ...
    if (fs.existsSync(contractsDir)) {
      const contracts = fs.readdirSync(contractsDir).filter(f => f.endsWith('.sol')).length;
      log(`   📜 Smart Contracts: ${contracts}`, 'white');
    }
    
    if (fs.existsSync(frontenDir)) {
      const components = fs.readdirSync(path.join(frontenDir, 'components')).filter(f => f.endsWith('.jsx')).length;
      log(`   🎨 Components: ${components}`, 'white');
    }
    
    if (fs.existsSync(testsDir)) {
      const tests = fs.readdirSync(testsDir).filter(f => f.endsWith('.test.js')).length;
...
`;
    ...
    log('💡 Make sure you have configured .env with valid credentials', 'yellow');
  }
}
